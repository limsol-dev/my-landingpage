-- =============================================
-- 달팽이 아지트 펜션 - 완전한 예약 스키마 설계 (수정된 버전)
-- PostgreSQL 생성된 컬럼 제약 해결
-- =============================================

-- 1. 기존 테이블 백업 (필요시)
CREATE TABLE IF NOT EXISTS reservations_backup AS SELECT * FROM reservations;

-- 2. 메인 예약 테이블 (PostgreSQL 호환 버전)
...
CREATE TABLE reservations (
  ...
  -- net_revenue를 모든 매출 필드를 직접 계산하여 해결
  net_revenue INTEGER GENERATED ALWAYS AS (
    bus_revenue + breakfast_revenue + meat_revenue + bbq_revenue + burner_revenue + room_revenue + facility_revenue + extra_revenue - refund_amount
  ) STORED, -- 순매출 (모든 매출 직접 계산)
...
-- 완료 메시지
SELECT '🎉 달팽이 아지트 펜션 - 완전한 예약 스키마 설계 완료! (PostgreSQL 호환)' as message;
SELECT '📋 포함된 주요 필드: 예약날짜, 결제완료날짜, 고객넘버링, 예약상태, 연락처, 예약목적, 확정인원, 기본/추가인원, 각종매출, 유입경로' as included_fields;
SELECT '🔧 수정사항: net_revenue 계산 방식 변경으로 PostgreSQL 호환성 해결' as fix_applied; 